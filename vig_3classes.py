...
class Decoder(nn.Module):
    def __init__(self, in_dim=768, out_dim=10, act='relu'):
        super().__init__()
        ...
        self.convs4=nn.Sequential(
            nn.ConvTranspose2d(in_dim//4,in_dim//8,kernel_size=2,stride=2),
            nn.Conv2d(in_dim//8, in_dim//8, 3, stride=1, padding=1),
        ...
        self.convs5=nn.Sequential(
            nn.ConvTranspose2d(in_dim//8,3,kernel_size=2,stride=2),
            nn.Conv2d(3, 3, 3, stride=1, padding=1),
            nn.BatchNorm2d(3),
            nn.Softmax(dim=1),
        )

        self.convs_decoder1=nn.Sequential(
            nn.Conv2d(in_dim, in_dim, 3, stride=2, padding=1),
            nn.BatchNorm2d(in_dim),
            act_layer(act),
        )

        self.convs_decoder2=nn.Sequential(
            nn.Conv2d(in_dim//2, in_dim, 5, stride=4, padding=1),
            nn.BatchNorm2d(in_dim),
            act_layer(act),
        )

        self.convs_decoder3=nn.Sequential(
            nn.Conv2d(in_dim//4, in_dim//2, 3, stride=2, padding=1),
            nn.BatchNorm2d(in_dim//2),
            act_layer(act),
        ...
        self.convs_decoder4=nn.Sequential(
            nn.Conv2d(in_dim//8, in_dim//4, 5, stride=4, padding=1),
            nn.BatchNorm2d(in_dim//4),
            act_layer(act),

            nn.Conv2d(in_dim//4, in_dim, 5, stride=4, padding=1),
            nn.BatchNorm2d(in_dim),
            act_layer(act),
        )
    ...
    def forward(self, x):
        B,_,H,W=x.shape
        output_feature=torch.zeros(B,0,H,W).cuda()
        x = self.convs1(x)
        feature=self.convs_decoder1(x)
        output_feature=torch.cat((output_feature,feature),dim=1)
        x = self.convs2(x)
        feature=self.convs_decoder2(x)
        output_feature=torch.cat((output_feature,feature),dim=1)
        x = self.convs3(x)
        feature=self.convs_decoder3(x)
        output_feature=torch.cat((output_feature,feature),dim=1)
        x = self.convs4(x)
        feature=self.convs_decoder4(x)
        output_feature=torch.cat((output_feature,feature),dim=1)
        x = self.convs5(x)

        # classification=self.classification_head(x)
        return x,output_feature#,classification
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.decoder=Decoder(in_dim=channels, out_dim=n_class)
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, channels, 32, 32))
        ...
        n_blocks=self.n_blocks+4

        self.output_decoder=Seq(
            nn.Conv2d(n_blocks*channels,n_blocks*channels//4,1),
            nn.BatchNorm2d(n_blocks*channels//4),
            act_layer(act),
            
            nn.Conv2d(n_blocks*channels//4,n_blocks*channels//8,1),
            nn.BatchNorm2d(n_blocks*channels//8),
            act_layer(act),
            nn.Conv2d(n_blocks*channels//8,channels,1),
    ...
    def forward(self, inputs):
        ...
        for i in range(self.n_blocks):
            x = self.backbone[i](x)
            output_feature=torch.cat((output_feature,x),dim=1)

        regression,feature=self.decoder(x)
        # print(feature.shape)
        output_feature=torch.cat((output_feature,feature),dim=1)
        # print(regression)
        output_feature=self.output_decoder(output_feature)
        # print(output_feature.shape)
        # x=F.gelu(x)

        return output_feature,regression
        # return x,regression,classification

        # return output_feature,regression,classification
...
def vig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=2, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            ...
            self.n_filters = 270 # number of channels of deep features
...
def vig_s_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=2, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            ...
            self.n_filters = 280#360#280#220#280#320 # number of channels of deep features
            self.n_classes = num_classes # Dimension of out_channels
            self.dropout = drop_rate # dropout rate
            self.use_dilation = True # use dilated knn or not
            self.epsilon = 0.2 # stochastic epsilon for gcn
